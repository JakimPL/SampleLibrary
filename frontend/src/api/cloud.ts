...
export type CloudCategory = components["schemas"]["CloudCategory"];
...
export async function getCloudCategories(): Promise<readonly CloudCategory[]> {
    return requestJson<readonly CloudCategory[]>("/cloud/categories");
}
...
export async function getCategoryTags(): Promise<readonly TagSummary[]> {
    return requestJson<readonly TagSummary[]>("/cloud/category-tags");
}
