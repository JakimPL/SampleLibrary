 ...
 *
 * A plain, unmodified click only highlights `entity`, staying on the current view; a modified
 * click (ctrl/cmd/shift/alt, or a non-primary button) is left alone so a row's own link can still
 * open in a new tab as usual. A double-click navigates to `entity`'s own route, which is what
 * actually focuses it -- the shell's route-param effect is the one place that dispatches a focus
 * action, so a pasted URL and a double-click both focus an entity through the identical mechanism.
 */
