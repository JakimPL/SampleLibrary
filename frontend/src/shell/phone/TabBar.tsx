...
import { END_LETTERS, useMorphStore } from "../../morph/morphStore";
 ...
 * the morph lives, carries a dot while a pair is being built, hollow with one end chosen and
 * filled once both are, and the letter of the end selected to take the next sample.
 */
export function TabBar({ tabs, activeTabId }: TabBarProps): ReactElement {
    ...
    const selectedEnd = useMorphStore((state) => state.selectedEnd);
        ...
        <nav className="tab-bar" aria-label="Sections">
            {tabs.map((tab) => (
                ...
                >
                    ...
                    {tab.id === CLOUD_TAB_ID && selectedEnd !== null && (
                        <span className="tab-bar-end" data-testid="morph-selected-end" aria-hidden>
                            {END_LETTERS[selectedEnd]}
                        </span>
                    )}
                    {tab.id === CLOUD_TAB_ID && selectedEnd === null && pairState !== "none" && (
                        <span
