...
def test_get_on_an_unknown_hash_returns_none(connection: Connection) -> None:
    assert PostgresSampleSpectralFeatureRepository(connection).get("f" * 64) is None
...
def test_a_feature_round_trips_through_get(connection: Connection, stored_sample: Sample) -> None:
...
def test_upserting_the_same_sample_again_replaces_its_vector(connection: Connection, stored_sample: Sample) -> None:
    repository = PostgresSampleSpectralFeatureRepository(connection)
