...
def client(connection: Connection, tmp_path: Path) -> Iterator[TestClient]:
    ...
    `create_app`'s own database URL is never actually opened once the dependencies are overridden,
    so it names no real database. `library_root` is a real temporary directory, since the audio and
