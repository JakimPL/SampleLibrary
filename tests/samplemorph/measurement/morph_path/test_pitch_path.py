...
def canonicalizer() -> Canonicalizer:
    return CANONICALIZER_REGISTRY[DEFAULT_CANONICALIZER_NAME]()
...
def test_frames_played_at_twice_the_rate_are_heard_an_octave_higher(canonicalizer: Canonicalizer) -> None:
    tone = harmonic_tone(FRAME_COUNT, frequency=220.0)[:, 0]

    lower = heard_pitch(tone, rate_hz=NOMINAL_WAV_RATE / 2.0, canonicalizer=canonicalizer)
    higher = heard_pitch(tone, rate_hz=NOMINAL_WAV_RATE, canonicalizer=canonicalizer)

    assert higher - lower == pytest.approx(OCTAVE_SEMITONES, abs=PITCH_TOLERANCE_SEMITONES)


def test_two_tones_a_fifth_apart_have_a_pitch_path(canonicalizer: Canonicalizer) -> None:
    first = harmonic_tone(FRAME_COUNT, frequency=220.0)[:, 0]
    second = harmonic_tone(FRAME_COUNT, frequency=330.0)[:, 0]

    assert is_pitched_pair(first, second, rate_hz=NOMINAL_WAV_RATE, canonicalizer=canonicalizer)


def test_one_note_twice_or_an_unpitched_end_leave_no_pitch_path(canonicalizer: Canonicalizer) -> None:
    tone = harmonic_tone(FRAME_COUNT, frequency=220.0)[:, 0]
    burst = noise_burst(FRAME_COUNT, seed=5)[:, 0]

    assert not is_pitched_pair(tone, tone, rate_hz=NOMINAL_WAV_RATE, canonicalizer=canonicalizer)
    assert not is_pitched_pair(tone, burst, rate_hz=NOMINAL_WAV_RATE, canonicalizer=canonicalizer)
