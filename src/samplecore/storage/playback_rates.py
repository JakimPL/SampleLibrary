...
def resolved_playback_rates(connection: Connection, sample_hashes: list[str]) -> dict[str, Rate | None]:
    ...
    the dominant rate its module occurrences and sample files declare answers for a sample the note
    events never reached; a sample with neither maps to None. Every hash asked for is present in the result.
    """
    occurrence_rates = PostgresSampleRepository(connection).rates_by_hash(sample_hashes)
