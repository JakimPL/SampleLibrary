...
def resolve_note_events(
    song: Song, *, module_id: int, cataloged_slots: frozenset[tuple[int, int]]
) -> tuple[NoteEvent, ...]:
    """Every key the song's patterns press, carrying the note and sample its instrument routes it to.

    ``cataloged_slots`` states which occurrences the catalog holds for this module, so a key routed
    ...
    """
    voices = addressable_voices(song)
    sample_slots = _sample_slots(voices, cataloged_slots=cataloged_slots)
...
def _sample_slots(voices: InstrumentVoices, *, cataloged_slots: frozenset[tuple[int, int]]) -> SampleSlots:
    ...
    return tuple(
        {
            ...
            if (instrument_index, slot) in cataloged_slots
        }
        for instrument_index, instrument in enumerate(voices.instruments)
    )
