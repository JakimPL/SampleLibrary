...
1. Unit tests may mock system boundaries (file I/O, external services, DuckDB connections where appropriate), but must not mock the domain logic that is the subject of the test. Integration tests must exercise real computation pipelines against real (synthetically built) data.
